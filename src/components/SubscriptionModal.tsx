...
export default function SubscriptionModal({ open, onClose, onSubscribed }: SubscriptionModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setStep('select');
      setSelectedTier(profile?.subscription_tier === 'pro' ? 'pro' : 'pro');
      setSenderMobile('');
      setReceiptCode('');
      setError('');
    }
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      const newCredits = selectedTier === 'pro' ? -1 : PRICING.starter.credits;
      await supabase
        .from('profiles')
        .update({ subscription_tier: selectedTier, credits: newCredits })
        .eq('id', userId);
  ...
  const tiers = [
    {
      ...
      icon: Zap,
      features: ['100 AI generations', 'All templates', 'All viewport sizes', 'ZIP export + share', 'Version history'],
    },
    {
      ...
      icon: Crown,
      features: ['Unlimited AI generations', 'Priority processing', 'Advanced templates', 'Team sharing', 'Priority support', 'Custom branding'],
      popular: true,
    },
  ];
            ...
            {profile?.subscription_tier && profile.subscription_tier !== 'free' && (
              <div className="mb-4 flex items-center gap-2 px-3 py-2 rounded-lg bg-green-50 border border-green-200">
                ...
                <span className="text-xs font-medium text-green-700">
                  Your {profile.subscription_tier.toUpperCase()} plan is active
                </span>
              </div>
            )}
            <div className="space-y-3">
              {tiers.map((tier) => {
                ...
                return (
                  ...
                  >
                    {tier.popular && (
                      <span className="absolute -top-2.5 right-4 text-[10px] font-bold text-white bg-orange-500 px-2 py-0.5 rounded-full">POPULAR</span>
                    )}
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isSelected ? 'bg-orange-500' : 'bg-slate-100'}`}>
                    ...
                    {isCurrent && (
                      <span className="inline-block mt-2 text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded">CURRENT PLAN</span>
                    )}
                  </button>
                );
              })}
          ...
          <div className="p-6">
            <div className="mb-5 p-4 rounded-xl bg-gradient-to-br from-orange-500 to-amber-500 text-white">
              ...
              <p className="text-xs text-orange-50 mb-3">
                Send {PRICING[selectedTier].price} EGP to the wallet number below via Orange Cash, then enter your details.
              </p>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">Your Sender Mobile Number</label>
                <input
                  type="tel"
                  value={senderMobile}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">Transaction Reference / Receipt Code</label>
                <input
                  type="text"
                  value={receiptCode}
            ...
            <div className="flex gap-2 mt-5">
              ...
              >
                {submitting ? (
                  ...
                  <><ShieldCheck size={16} /> Submit & Activate</>
                )}
              </button>
            </div>
          </div>
